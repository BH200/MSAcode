...
def dijkstra(graph, start):
    ...
    #set next to the node just add it to confirmed  
    next = start 

    #while the tentative dictionary still has nodes 
    while tentative:
        shortest_distance = math.inf
        #select node from tenative dictionary with the lowest cost 
    ...
    # remove next node from tentative dictionary
        tentative.pop(next)
    #place next nodes neighbors on tentitave list.
        for node, distance in graph[next].items():
            cost = confirmed[next] + distance 
            #if the neighbor is not in tentaive ,add to tentative
